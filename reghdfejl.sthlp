...
{pstd}
To run, {cmd:reghdfejl} requires that the Stata command {cmd:jl} be installed; "{stata ssc install jl}" should suffice. It also needs
...
{pstd}
More so than for most packages, the author of this one stands on the shoulders of giants. {cmd:reghdfejl} is merely a wrapper
